...
const GetNewsAndQuestionsOutputSchema = z.object({
  article: z.string().describe('The full text content of the news article in English, as provided by the news tool. Prioritize using the "Full Content" field from the tool. If "Full Content" is unavailable or very brief (less than a few sentences), then use the "Description Snippet". This field should contain the direct text from the tool, not a summary generated by this flow. If no article can be found, this field should state that.'),
...
Your goal is to prepare this article for a reading comprehension exercise by extracting the text and generating questions.
...
1.  **Process Article Data (if available):**
    {{#if toolOutput}}
    a.  For the 'article' field in your JSON output:
        *   **CRITICAL: Use the 'Full Content' ({{{toolOutput.content}}}) directly if it is available and substantial (more than a few sentences).**
        *   If 'Full Content' is unavailable, empty, or very short, then use the 'Description Snippet' ({{{toolOutput.description}}}) as the 'article' text.
        *   **DO NOT summarize or shorten the text yourself if 'Full Content' or 'Description Snippet' is provided. Use the text from the tool as directly as possible.**
    ...
    e.  Based *only* on the article text you have decided to use (either full content or description snippet), generate 1 or 2 clear comprehension questions. These questions should test understanding of the main points of that text.
    {{else}}
...
Focus on using the provided text ('Full Content' or 'Description Snippet') for the article. Do not invent details or generate summaries of the tool's output.
